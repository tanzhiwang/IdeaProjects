...
public class GoodsService {
    ...
    public PageResult<Spu> querySpuByPage(Integer page, Integer rows, Boolean saleable, String key) {
        ...
        if (CollectionUtils.isEmpty(spus)){
            throw new LyException(ExceptionEnum.GOODS_NOT_FUND);
        }
        //解析分类和品牌的名称
    ...
    private void loadCategoryAndBrandName(List<Spu> spus) {
        for (Spu spu : spus) {
            ...
            List<String> names = categoryService.queryByIds(Arrays.asList(spu.getCid1(), spu.getCid2(), spu.getCid3()))
                    .stream().map(Category::getName).collect(Collectors.toList());//字符串拼接会占过多内存
    ...
    public void saveGoods(Spu spu) {
        //新增spu
        spu.setId(null);//id是自增长的
        spu.setCreateTime(new Date());
        spu.setLastUpdateTime(spu.getCreateTime());
        spu.setSaleable(true);//默认上架
        spu.setValid(false);//逻辑删除为否
    ...
    public SpuDetail queryDetailById(Long spuId) {
        ...
        if(detail==null){
            throw new LyException(ExceptionEnum.GOODS_DETAIL_NOT_FUND);
        }
        return detail;
    }
...
//            if(stock==null){
//                throw new LyException(ExceptionEnum.GOODS_STOCK_NOT_FUND);
//            }
